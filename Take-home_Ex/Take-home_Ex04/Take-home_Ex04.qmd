...
```{r}
climate <- data.frame(lapply(climate, function(x) gsub("—", NA, x)), stringsAsFactors=F)
```
...
## 4.2 Filling gaps with non-zero means
...
Then, for each station, we find the months where all data is missing.
...
g <- ggplot(data = result_df) + 
  geom_line(aes(x=date, y=.data[[target_variable]])) +
  geom_line(aes(x=date, y=fit), color="blue")

if (validation) {
  g <- g + geom_line(aes(x=date, y=val), color="red")
}
