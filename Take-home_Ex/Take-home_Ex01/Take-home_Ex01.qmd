...
```{r}
pacman::p_load(tidyverse, haven, ggplot2, ggpubr)
```
...
```{r}
df$common_needs <- select(stu_qqq_SG, contains("ST250Q")) %>% mutate(across(contains("ST250Q"), function(x) x%%2)) %>% rowSums() %>% factor()
# %>% mutate_all(~replace(., is.na(.), 0))
```

```{r}
df$luxury_items <- select(stu_qqq_SG, contains("ST251Q")) %>% 
  select(-contains("03") & -contains("04")) %>% 
  mutate(across(contains("ST251Q"), function(x) if_else(x>1, 1, 0)))
```
...
```{r}
ggplot(na.omit(df), aes(x=common_needs, y=combined_grades)) + geom_boxplot()
